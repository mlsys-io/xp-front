  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-3xl px-8 py-12">
        ...
        {/* 7. Phase 1 supply — narrative paragraph, not numbers. */}
        <Section
          eyebrow="reuse"
        ...
        <div className="border-t border-gray-200 pt-8 mt-4 text-center">
          ...
          <p className="mt-2 text-[11px] text-gray-400 uppercase tracking-widest">
...
// Section header with the small eyebrow label + title + opening paragraph.
// Kept as a local component because the rhythm repeats seven times on the
// page; pulling it out keeps the body of <Learn /> scannable.
function Section({
  eyebrow, title, body,
}: {
  eyebrow: string;
...
// Kind card — used by the "browse by kind" row. Visual weight matches
// RepoCard but with a glyph in place of the repo metadata.
function KindCard({
  to, glyph, label, sub,
}: {
  to: string;
...
// Inline non-interactive search-bar mock. Reuses the visual of the real
// SearchBar without wiring it up — clicking does nothing, which is
// deliberate; the live SearchBar at the top of the nav handles real
// queries. This is a pure illustration of what the experience looks
// like with an example query already typed.
function SearchMock() {
  return (
...
// Author-badge live row — pairs a label (the email behind the sub)
// with the actual rendered AuthorBadge. The owner_subs are real:
// the first is admin@lumid.market (first-party tier), the second is
// admin@lum.id (verified tier), the third is a synthetic UUID that
// falls through to community. If the tier-classification logic ever
// changes, this preview reflects it for free.
function BadgeRow({
  label, owner_sub,
}: {
  label: string;
